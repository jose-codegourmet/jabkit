...
Run `pnpm --filter @jabkit/retro dev` from the root. Use the same filter with `build`, `start`, or `typecheck`. This app owns `app/globals.css` (Tailwind v4), `app/theme.css` (tokens/type), `app/style.module.css` (layout), and its public assets.
