...
**2026-09-15 update:** The runtime now lives in `apps/editorial` with root `/` and app-owned Tailwind configuration. The older ticket text below is historical. Use [the current app guide](../../docs/standalone-design-systems.md) for paths/deployment and [PROMPTS_FOR_IMAGES.md](../../PROMPTS_FOR_IMAGES.md) for Jose-run Higgsfield generation, logos, CTA artwork, and backgrounds. The agent does not generate images.
