...
Start with real representative media and remove stock illustration assumptions. Replace Button's aura and brightness effect with a plain surface transition. Replace Input's small fixed height and rounded corners. Review gallery31 or projects11 for collection layouts, but flatten unnecessary effects. An over-image navbar needs an opaque fallback and cannot rely on inherited image colors for contrast.
