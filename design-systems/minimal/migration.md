...
Remove Button's primary shadow and brightness filter; keep the focus ring. Input currently uses rounded-lg and h-8, so a radius-token swap is insufficient: replace those local shape and height choices. Use an unboxed figure rather than a prebuilt promotional card. Review projects11 or gallery31 as media candidates; inspect their motion before reuse.
