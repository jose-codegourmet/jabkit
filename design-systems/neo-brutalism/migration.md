# Applying Neo-brutalism to JabKit
...
Replace Button's blurred primary shadow with the shared hard-shadow token and replace its brightness hover with an explicit state. Override Input's rounded-lg and h-8 locally after installation. Use a new semantic card composition if a candidate carries incompatible glow or perspective effects; do not force spotlight-card into a flat poster. Preserve the Button variant API.
