...
Root scripts that matter (`package.json`): `dev` (showcase), `storybook`, `lint`, `typecheck`, `check:conventions`, `registry:build`, `registry:verify`, `previews:build`, `previews:verify`, and the composite gate `check`.
