...
| --- | --- | --- |
| `packages/ui` | `@jabkit/ui` | Component source. 39 components under `src/atoms` (13), `src/marketing` (21), `src/dashboard` (5). Owns `src/lib/cn.ts`, `src/lib/theme.ts`, and Storybook in `.storybook/`. No root barrel. |
...
`pnpm check` is `pnpm lint && pnpm typecheck && pnpm check:conventions && pnpm registry:verify`.
