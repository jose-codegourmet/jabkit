...
| `apps/<system>/public/assets/design-systems/<system>/**` | No (user-provided Higgsfield outputs or browser capture + local optimize) | **Committed** | Sample photography plus optional `catalogue-cover.webp` (`kind: "browser-capture"`). Provenance JSON is hand-authored from real jobs or captures. Not `sources.json`. Never write sample pages into `public/previews/`. |
