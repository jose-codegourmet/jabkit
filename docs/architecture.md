...
Root scripts that matter (`package.json`): `dev` (showcase), `storybook`, `lint`, `typecheck`, `check:conventions`, `check:design-system-assets`, `registry:build`, `registry:verify`, `previews:build`, `previews:verify`, and the composite gate `check`.
...
| `apps/showcase/public/previews/*` | Yes, by `pnpm previews:build` | **Committed** | Still WebP plus optional GIF. Never edit by hand. `pnpm previews:verify` fails if hashes or files are stale, and prints marketing coverage by component and story. |
| `apps/showcase/public/assets/*.webp` and `sources.json` | Yes, by `pnpm assets:vendor` | **Committed** | Library Unsplash/Simple Icons rehosts. Never edit by hand. |
| `apps/showcase/public/assets/design-systems/**` | No (Higgsfield MCP or browser capture + local optimize) | **Committed** | Sample photography plus optional `catalogue-cover.webp` (`kind: "browser-capture"`). Provenance JSON is hand-authored from real jobs or captures. Not `sources.json`. Never write sample pages into `public/previews/`. |
...
`pnpm check` is `pnpm lint && pnpm typecheck && pnpm check:conventions && pnpm check:design-system-assets && pnpm registry:verify && pnpm previews:verify`.
...
- Do not add Higgsfield sample files to `sources.json` or run them through `assets:vendor`.
